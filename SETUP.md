...
## 7. Point the public website at the same Firebase project
...
- **Site** — root `arditransportation`, env: the two `NEXT_PUBLIC_FIREBASE_*`
  vars, plus `REVALIDATE_SECRET` / `ADMIN_ORIGIN` if you did step 8.
...
| Symptom | Cause |
|---|---|
| "Missing or insufficient permissions" on save | Step 5 — UID not in the rules allow-list, or rules not deployed |
...
| "This account (…) is not in ADMIN_UIDS" | `ADMIN_UIDS` is set and doesn't include the account you signed in as. The message names your real UID — paste that in, or clear `ADMIN_UIDS` entirely. Restart the server after editing `.env.local` |
...
| Site still shows old content | Wait 60s, or set up step 8. On a host, confirm the site's env vars are set for the deployed environment |
